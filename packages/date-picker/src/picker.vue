<template>
    ...
    <div
        class="m-date-editor m-range-editor m-input__inner"
        :class="[
            'm-date-editor--' + type,
            pickerSize ? `m-range-editor--${ pickerSize }` : '',
        ...
        v-clickoutside="handleClose"
        v-else>
        <i :class="['m-input__icon', 'm-range__icon', triggerClass]"></i>
        <input
            autocomplete="off"
        ...
        <slot name="range-separator">
            <span class="m-range-separator">{{ rangeSeparator }}</span>
        </slot>
        <input
            autocomplete="off"
        ...
        <i
           @click="handleClickIcon"
            v-if="haveTrigger"
            :class="[showClose ? '' + clearIcon : '']"
            class="m-input__icon m-range__close-icon">
        </i>
    </div>
</template>
...
import { formatDate, isDateObject, parseDate, getWeekNumber } from 'melody-ui/src/utils/date-util'
...
export default {
    ...
    computed: {
        ...
        triggerClass() {
            return this.prefixIcon || (this.type.indexOf('time') !== -1 ? 'm-icon-time' : 'm0icon-date')
        },
