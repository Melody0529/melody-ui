<template>
    <transition name="m-zoom-in-top" @after-leave="$emit('dodestroy')">>
        <div
            v-show="visible"
            class="m-time-panel m-popper"
            :class="popperClass">
            <div class="m-time-panel__content" :class="{ 'has-second': showSeconds }">
                <time-spinner
                    ref="spinner"
                     @change="handleChange"
...
import { clearMilliseconds, isDate, timeWithinRange, limitTimeRange } from 'melody-ui/src/utils/date-util'
...
export default {
    ...
    computed: {
        ...
        useArrow() {
            return this.arrowControl || false
        },
        amPmMode() {
