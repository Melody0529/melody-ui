<template>
    <transition name="m-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('dodestroy')">
        <div
            ref="popper"
            v-show="visible"
            :style="{ width: width + 'px' }"
            :class="popperClass"
            class="m-picker-panel time-select m-popper">
            <m-scrollbar noresize wrap-class="m-picker-panel__content">
...
export default {
    ...
    methods: {
        ...
        scrollToOption(selector = '.selected') {
            const menu = this.$refs.popper.querySelector('.m-picker-panel__content')
        ...
        isValidValue(date) {
            return this.items.filter(item => !item.disabled).map(item => item.value) !== -1
        },
