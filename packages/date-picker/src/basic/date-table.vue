...
<script>
import { getFirstDayOfMonth, getDayCountOfMonth, getWeekNumber, getStartDateOfMonth, prevDate, nextDate, isDate, clearTime as _clearTime } from 'melody-ui/src/utils/date-util'
import Locale from 'melody-ui/src/mixins/locale'
import { arrayFindIndex, arrayFind, coerceTruthyValueToArray } from 'melody-ui/src/utils/util'
        ...
        rows() {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                for (let j = 0; j < 7; j++) {
                    let cell = row[j]
                    if (!cell) {
                        cell = { row: i, column: j, type: 'normal', isRange: false, start: false, end: false }
                    }
