...
<script>
  export default {
    data() {
      return {
        value: null,
        iconClasses: ['m-icon-heart-fill', 'm-icon-heart-fill', 'm-icon-heart-fill'] // 等同于 { 2: 'icon-rate-face-1', 4: { value: 'icon-rate-face-2', excluded: true }, 5: 'icon-rate-face-3' }
      }
    }
  }
</script>
...
| icon-classes | icon 的类名。若传入数组，共有 3 个元素，为 3 个分段所对应的类名；若传入对象，可自定义分段，键名为分段的界限值，键值为对应的类名 | array/object | — | ['m-icon-star-fill', 'm-icon-star-fill','m-icon-star-fill'] |
| void-icon-class | 未选中 icon 的类名 | string | — | m-icon-star |
| disabled-void-icon-class | 只读时未选中 icon 的类名 | string | — | m-icon-star-fill |
